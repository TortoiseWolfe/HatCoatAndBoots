...
```
origin  git@github.com:TortoiseWolfe/HatCoatAndBoots.git
```
...
```bash
git remote set-url origin git@github.com:TortoiseWolfe/HatCoatAndBoots.git
```
...
tmux capture-pane -t hatcoatandboots:18 -p | tail -20

# Check if stuck on permission prompt
tmux capture-pane -t hatcoatandboots:$i -p | grep -q "Do you want to proceed"
```
