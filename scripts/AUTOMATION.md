...
```
origin  git@github.com:TortoiseWolfe/HatsCoatsAndBoots.git
```
...
```bash
git remote set-url origin git@github.com:TortoiseWolfe/HatsCoatsAndBoots.git
```
...
tmux capture-pane -t hatscoatsandboots:18 -p | tail -20

# Check if stuck on permission prompt
tmux capture-pane -t hatscoatsandboots:$i -p | grep -q "Do you want to proceed"
```
