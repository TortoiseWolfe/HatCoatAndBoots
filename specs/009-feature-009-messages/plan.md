...
docker compose exec hatcoatandboots pnpm run type-check  # No any type errors
docker compose exec hatcoatandboots pnpm run lint        # No ESLint warnings
docker compose exec hatcoatandboots pnpm test            # All tests pass

# Final verification
docker compose exec hatcoatandboots pnpm run build       # Production build clean
```
...
| SC-006: A11y passes            | `docker compose exec hatcoatandboots pnpm run test:a11y`                    |
