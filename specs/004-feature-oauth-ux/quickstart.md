...
```bash
docker compose exec hatcoatandboots pnpm test src/lib/auth/oauth-utils.test.ts
```
...
```bash
# Unit tests
docker compose exec hatcoatandboots pnpm test oauth-utils

# E2E test (if available)
docker compose exec hatcoatandboots pnpm exec playwright test welcome-message
```
