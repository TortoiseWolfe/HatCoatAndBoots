...
```bash
docker compose exec hatscoatsandboots pnpm test src/lib/auth/oauth-utils.test.ts
```
...
```bash
# Unit tests
docker compose exec hatscoatsandboots pnpm test oauth-utils

# E2E test (if available)
docker compose exec hatscoatsandboots pnpm exec playwright test welcome-message
```
