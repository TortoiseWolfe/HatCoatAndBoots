...
The hand-drawn "hammer + anvil + DaisyUI accent orbs" composition was replaced with the three canonical HatsCoatsAndBoots brand SVGs inlined as `<symbol>` definitions in the SVG's `<defs>`, then referenced via `<use href="#brand-..."` at the correct layered positions.

Composition follows the canonical layering rules from `src/components/atomic/SpinningLogo/LayeredHatsCoatsAndBootsLogo.tsx`:
...
| 2 (MIDDLE) | `#brand-cog` (mirrors `public/hatscoatsandboots-logo.svg`)      | centered                               | 100%                 |
