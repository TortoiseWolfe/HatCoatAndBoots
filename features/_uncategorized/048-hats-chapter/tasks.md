...
- [ ] T041 [US3] Create `src/app/book/layout.tsx` (chapter chrome + prev/next nav) and `src/app/book/page.tsx` — **the index = the SAME shared viewer mounted with `chapterFocus={null}` (neutral no-focus state)**: full balanced building (no region dimmed), chapter-focus tabs inactive, guided-views/toggles inert with a "pick a chapter to start" prompt; NOT a separate card layout. Reuses `LayeredDiagram` + the rails. Per the signed-off `01-book-index` wireframe (FR-007).
