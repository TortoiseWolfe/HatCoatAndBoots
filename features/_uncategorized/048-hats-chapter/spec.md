...
- Book index = **the same shared viewer with no chapter focus** (neutral landing): [`wireframes/01-book-index.svg`](./wireframes/01-book-index.svg) (light)
...
**One UI everywhere.** All four screens are the _same_ 3-column viewer (chapter-focus rail + guided views | the shared building | per-layer toggles + language); they differ only by which chapter focus is active and the per-region opacity. The `/book` index is NOT a separate card/landing layout — it is the viewer in a **neutral no-focus state**: no chapter tab active, the whole building shown at full opacity (no region dimmed), the guided-views list disabled with a "pick a chapter to start" prompt, and an intro explanation. Choosing a chapter-focus tab (Hat / Coat / Boots) IS the navigation. These wireframes are spec constraints — implementation should match their one-building/shared-coordinate layout, the chapter-focus model, the 3-region building, and the control structure. Deviations require spec revision.
...
- **FR-013a**: Any custom display typography used in the drawing or chapter MUST be served from within the site itself (no third-party request) and MUST declare a system-font fallback, so that if the custom font fails to load, all labels and text remain legible. (The custom display font is Latin-only; the system-CJK font strategy for Simplified Chinese is part of carved-out issue [#2](https://github.com/TortoiseWolfe/HatCoatAndBoots/issues/2), but using a self-hosted Latin-only font here keeps that future addition rework-free.)
- **FR-014**: At viewport widths **≤ 640 px** (the project's Tailwind `sm` breakpoint) the three regions (guided views, drawing, element toggles) MUST reflow into a single readable column, and every interactive control MUST present a touch target of at least 44×44 pixels (at all widths).
- **FR-015**: The reader's path through the chapter MUST be expressible as the reading sequence: bare wall (problem) → add overhang → summer sun blocked → winter sun admitted → rain shed → measurements — so the guided views and explanations reinforce a one-variable-at-a-time reasoning process. **This 6-beat reasoning arc is the _pedagogical sequence_, not a requirement for six named views.** It is realized by the four named guided views of FR-002 (bare wall = the problem; roof line = add overhang; how it sheds water = rain shed; everything = the full picture) combined with the per-element toggles (FR-004) for the summer-sun-blocked, winter-sun-admitted, and measurements beats. FR-015 introduces no additional named views; FR-002's four views are the complete set.
...
- **Drawing Element (Layer)**: one named, individually-showable part of the building, tagged with the Region it belongs to. The canonical inventory is **6 SVG layers + 1 HTML labels overlay = 7 toggleable Drawing Elements**: the SVG layers are `wall` (envelope), `foundation` (footing), `overhang` (roof/Hat), `sun-high` (summer), `sun-low` (winter), and `rain`; the 7th element is the `labels` overlay. Each carries a human-readable name, an on/off default, and a stacking order in the shared coordinate space. **Screen-reader treatment**: the six pictorial SVG layers are decorative (`alt=""` / `aria-hidden`) — they carry no text alternative; all meaning reaches assistive technology through (a) the HTML **labels** overlay and (b) the control names + the `aria-live` explanation. The **labels** element is therefore the only element with reader-facing words: its dimension/angle callouts are HTML text positioned over the drawing (not baked into the SVG) — authored in English this slice, structured so the carved-out multilingual feature (issue [#2](https://github.com/TortoiseWolfe/HatCoatAndBoots/issues/2)) can translate them without re-authoring the artwork.
...
- **SC-004**: The chapter loads as a static page with no server dependency and a first-load JS payload that MUST NOT exceed the project's **150 KB** budget (hard cap), with a **target of ≤ 100 KB**; the full set of drawing SVGs totals ~11 KB.
...
- **SC-006**: At a viewport width ≤ 640 px (Tailwind `sm`), all three regions are reachable in a single scroll column and every control meets the 44×44 px touch-target minimum.
...
- **SC-009**: Switching the chapter focus (Hat → Coat → Boots) never moves any building element on screen — the roof, envelope, and foundation stay aligned and registered in the same coordinate space across all focuses (the building reads as one structure, not three drawings). _(Renumbered from SC-011; the former SC-009/SC-010 were removed when multilingual scope was carved out to issue #2.)_

## Assumptions

- **Audience & tone**: readers are 13–18; language builds intuition and respects the reader (not babyish, not jargon-heavy).
- **Language**: ships **English only**. Multilingual (EN + ES + Simplified Chinese, language switcher, URL-reflected language, translated labels, system-CJK font strategy, draft-translation discipline) is carved out to its own feature, issue [#2](https://github.com/TortoiseWolfe/HatCoatAndBoots/issues/2) (`049-i18n-multilingual`), with this chapter as its first consumer. This slice keeps the labels as an HTML overlay and authors text as externally-referenced strings so 049 has no rework.
...
- **All multilingual work** — any language other than English, the language switcher, URL-reflected language, translated drawing labels, the system-CJK font strategy, and draft/native-reviewed Spanish & Chinese copy. This is carved out to issue [#2](https://github.com/TortoiseWolfe/HatCoatAndBoots/issues/2) (`049-i18n-multilingual`), which depends on this slice.
