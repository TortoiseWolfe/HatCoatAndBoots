...
| IV. Docker-First Development                     | ✅         | All commands run via `docker compose exec hatscoatsandboots ...`. Tests run inside the container. No host pnpm.                                                                                                                                            |
...
- Credential Management API: `navigator.credentials.store(new PasswordCredential(...))` works in both page and modal contexts. Verify in browser during implementation per watch-out 2 in `~/.claude/plans/continue-the-hatscoatsandboots-ticklish-deer.md`.
...
docker compose exec hatscoatsandboots python3 .specify/extensions/wireframe/scripts/validate.py \
...
docker compose exec hatscoatsandboots pnpm exec playwright test tests/e2e/messaging/oauth-setup-modal.spec.ts
