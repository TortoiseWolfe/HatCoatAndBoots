...
tmux send-keys -t hatcoatandboots:RoleName "/clear"
tmux send-keys -t hatcoatandboots:RoleName "/exit"

# CORRECT - command is actually executed:
tmux send-keys -t hatcoatandboots:RoleName "/clear" Enter
sleep 3
tmux send-keys -t hatcoatandboots:RoleName "/prime [role]" Enter

# CORRECT - exit is actually executed:
tmux send-keys -t hatcoatandboots:RoleName "/exit" Enter
```
...
tmux list-windows -t hatcoatandboots -F "#{window_index}:#{window_name}" | grep RoleName

# Send to terminal by NAME (not window number)
tmux send-keys -t hatcoatandboots:Toolsmith "command" Enter
```
...
└─────────────────────────────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│  TMUX SESSION "hatcoatandboots" (26 windows)                   │
│  Windows named by ROLE, not number                          │
│  Access via: tmux send-keys -t hatcoatandboots:RoleName        │
└─────────────────────────────────────────────────────────────┘
```
...
tmux capture-pane -t hatcoatandboots:Toolsmith -p | tail -30

# 5. Check completion
grep -c '✅' docs/interoffice/audits/*.md

# 6. Attach to observe (Ctrl+b d to detach)
tmux attach -t hatcoatandboots

# 7. Kill session when done
tmux kill-session -t hatcoatandboots
```
...
tmux list-windows -t hatcoatandboots -F "#{window_index}:#{window_name}"

# Check specific terminal by name
tmux capture-pane -t hatcoatandboots:Developer -p | tail -30

# Find stuck terminals (waiting on permission)
for win in $(tmux list-windows -t hatcoatandboots -F "#{window_name}"); do
  if tmux capture-pane -t hatcoatandboots:$win -p | grep -q "Do you want to proceed"; then
