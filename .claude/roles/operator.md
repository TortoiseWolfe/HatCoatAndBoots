...
Manage 26 worker terminals inside the `hatcoatandboots` tmux session.
...
tmux send-keys -t hatcoatandboots:RoleName "command" Enter
```
...
# Attach/Detach
tmux attach -t hatcoatandboots   # Ctrl+b d to detach
```
...
tmux send-keys -t hatcoatandboots:WireframeGenerator1 "/clear" Enter
sleep 2
tmux send-keys -t hatcoatandboots:WireframeGenerator1 "/prime wireframe-generator" Enter
```
