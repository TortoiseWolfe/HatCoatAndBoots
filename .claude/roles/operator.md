...
Manage 26 worker terminals inside the `hatscoatsandboots` tmux session.
...
tmux send-keys -t hatscoatsandboots:RoleName "command" Enter
```
...
# Attach/Detach
tmux attach -t hatscoatsandboots   # Ctrl+b d to detach
```
...
tmux send-keys -t hatscoatsandboots:WireframeGenerator1 "/clear" Enter
sleep 2
tmux send-keys -t hatscoatsandboots:WireframeGenerator1 "/prime wireframe-generator" Enter
```
