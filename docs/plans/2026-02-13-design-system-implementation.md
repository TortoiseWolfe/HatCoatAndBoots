...
**Goal:** Clean up the component hierarchy, upgrade Storybook to v10, build a custom HatsCoatsAndBoots DaisyUI theme (dark default + light variant), add theme template tooling for forks, then redesign components with the new brand identity.
...
**Important:** HatsCoatsAndBoots runs inside Docker. All pnpm/storybook commands run via `docker compose exec hatscoatsandboots <command>` unless noted otherwise.
...
Run: `docker compose exec hatscoatsandboots ls src/components/atomic/`
...
**Step 3: Audit HatsCoatsAndBoots-specific extras**

HatsCoatsAndBoots has 33 atomic dirs vs SpokeToWork's 29. Check the 4+ extras for misplacement. Components that compose multiple primitives belong in molecular/. Full interactive interfaces belong in organisms/.
...
Run: `docker compose exec hatscoatsandboots mkdir -p src/lib/analytics && mv src/components/atomic/GoogleAnalytics src/lib/analytics/`
...
Run: `docker compose exec hatscoatsandboots pnpm run type-check`
Expected: No errors
...
Run: `docker compose exec hatscoatsandboots mv src/components/atomic/DiceTray src/components/organisms/`
...
```bash
docker compose exec hatscoatsandboots bash -c "
...
Run: `docker compose exec hatscoatsandboots pnpm run type-check && pnpm run build`
Expected: Clean build with no errors

**Step 6: Commit**

```bash
cd ~/repos/HatsCoatsAndBoots
git add -A
...
Run: `docker compose exec hatscoatsandboots node --version`
Expected: v22.x (meets Storybook 10 requirement of 20.19+)

**Step 2: Verify current Storybook version**

Run: `docker compose exec hatscoatsandboots npx storybook --version`
Expected: 9.1.x

**Step 3: Check which stories currently render**

Run: `docker compose exec hatscoatsandboots npx storybook build --test 2>&1 | tail -30`
...
Run: `docker compose exec hatscoatsandboots npx storybook@latest upgrade`
...
Run: `docker compose exec hatscoatsandboots pnpm install`
...
Run: `docker compose exec hatscoatsandboots grep -r "addon-essentials" .storybook/main.ts`
...
Run: `docker compose exec hatscoatsandboots pnpm add -D @storybook/addon-essentials`
...
Run: `docker compose exec hatscoatsandboots pnpm list @storybook/test`
Expected: 9.0.0-alpha.2

**Step 2: Upgrade to stable**

Run: `docker compose exec hatscoatsandboots pnpm add -D @storybook/test@latest`
...
Run: `docker compose exec hatscoatsandboots npx storybook build --test 2>&1`
...
## Phase 2: HatsCoatsAndBoots Brand Theme ✅ COMPLETE
...
Run: `docker compose exec hatscoatsandboots pnpm list daisyui`
...
### Task 11: Define hatscoatsandboots-dark theme
...
```css
[data-theme='hatscoatsandboots-dark'] {
...
```css
@plugin "daisyui" {
  themes: hatscoatsandboots-dark, hatscoatsandboots-light, light, dark;
}
```
...
git commit -m "feat: add hatscoatsandboots-dark custom DaisyUI theme"
```

---

### Task 12: Define hatscoatsandboots-light theme
...
```css
[data-theme='hatscoatsandboots-light'] {
...
cd ~/repos/HatsCoatsAndBoots
git add src/app/globals.css
git commit -m "feat: add hatscoatsandboots-light custom DaisyUI theme"
```
...
- Modify: `src/components/ThemeScript.tsx` (change default from 'light' to 'hatscoatsandboots-dark')
- Modify: `.storybook/preview.tsx` (add custom themes to switcher, set default)

**Step 1: Update ThemeScript default**

In `src/components/ThemeScript.tsx`, change the fallback theme from `'light'` to `'hatscoatsandboots-dark'`.
...
Run: `docker compose exec hatscoatsandboots npx storybook build --test 2>&1 | tail -10`
Expected: Clean build
...
git commit -m "feat: set hatscoatsandboots-dark as default theme"
```
...
**Step 1: Add hatscoatsandboots-dark to map control selectors**

The existing CSS has selectors like `[data-theme='dark'] .maplibregl-ctrl-group`. Add `[data-theme='hatscoatsandboots-dark']` to each group of dark theme selectors for both MapLibre and Leaflet controls.
...
cd ~/repos/HatsCoatsAndBoots
git add src/app/globals.css
git commit -m "fix: add hatscoatsandboots-dark to map control dark theme selectors"
```
...
Run: `docker compose exec hatscoatsandboots npx storybook dev -p 6006 --no-open`

**Step 2: Review Button in both themes**

Open `http://localhost:6006` and switch between hatscoatsandboots-dark and hatscoatsandboots-light themes. Document what needs to change:
...
git commit -m "style: update Button component for HatsCoatsAndBoots theme"
```
...
- **Task 22:** AnimatedLogo / SpinningLogo (HatsCoatsAndBoots's mallet)

### Tasks 23-24: HatsCoatsAndBoots-specific components
...
- Phase 3 (theme template system) is unique to HatsCoatsAndBoots as a template project.
