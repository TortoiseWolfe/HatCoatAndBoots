...
**Goal:** Clean up the component hierarchy, upgrade Storybook to v10, build a custom HatCoatAndBoots DaisyUI theme (dark default + light variant), add theme template tooling for forks, then redesign components with the new brand identity.
...
**Important:** HatCoatAndBoots runs inside Docker. All pnpm/storybook commands run via `docker compose exec hatcoatandboots <command>` unless noted otherwise.
...
Run: `docker compose exec hatcoatandboots ls src/components/atomic/`
...
**Step 3: Audit HatCoatAndBoots-specific extras**

HatCoatAndBoots has 33 atomic dirs vs SpokeToWork's 29. Check the 4+ extras for misplacement. Components that compose multiple primitives belong in molecular/. Full interactive interfaces belong in organisms/.
...
Run: `docker compose exec hatcoatandboots mkdir -p src/lib/analytics && mv src/components/atomic/GoogleAnalytics src/lib/analytics/`
...
Run: `docker compose exec hatcoatandboots pnpm run type-check`
Expected: No errors
...
Run: `docker compose exec hatcoatandboots mv src/components/atomic/DiceTray src/components/organisms/`
...
```bash
docker compose exec hatcoatandboots bash -c "
...
Run: `docker compose exec hatcoatandboots pnpm run type-check && pnpm run build`
Expected: Clean build with no errors

**Step 6: Commit**

```bash
cd ~/repos/HatCoatAndBoots
git add -A
...
Run: `docker compose exec hatcoatandboots node --version`
Expected: v22.x (meets Storybook 10 requirement of 20.19+)

**Step 2: Verify current Storybook version**

Run: `docker compose exec hatcoatandboots npx storybook --version`
Expected: 9.1.x

**Step 3: Check which stories currently render**

Run: `docker compose exec hatcoatandboots npx storybook build --test 2>&1 | tail -30`
...
Run: `docker compose exec hatcoatandboots npx storybook@latest upgrade`
...
Run: `docker compose exec hatcoatandboots pnpm install`
...
Run: `docker compose exec hatcoatandboots grep -r "addon-essentials" .storybook/main.ts`
...
Run: `docker compose exec hatcoatandboots pnpm add -D @storybook/addon-essentials`
...
Run: `docker compose exec hatcoatandboots pnpm list @storybook/test`
Expected: 9.0.0-alpha.2

**Step 2: Upgrade to stable**

Run: `docker compose exec hatcoatandboots pnpm add -D @storybook/test@latest`
...
Run: `docker compose exec hatcoatandboots npx storybook build --test 2>&1`
...
## Phase 2: HatCoatAndBoots Brand Theme ✅ COMPLETE
...
Run: `docker compose exec hatcoatandboots pnpm list daisyui`
...
### Task 11: Define hatcoatandboots-dark theme
...
```css
[data-theme='hatcoatandboots-dark'] {
...
```css
@plugin "daisyui" {
  themes: hatcoatandboots-dark, hatcoatandboots-light, light, dark;
}
```
...
git commit -m "feat: add hatcoatandboots-dark custom DaisyUI theme"
```

---

### Task 12: Define hatcoatandboots-light theme
...
```css
[data-theme='hatcoatandboots-light'] {
...
cd ~/repos/HatCoatAndBoots
git add src/app/globals.css
git commit -m "feat: add hatcoatandboots-light custom DaisyUI theme"
```
...
- Modify: `src/components/ThemeScript.tsx` (change default from 'light' to 'hatcoatandboots-dark')
- Modify: `.storybook/preview.tsx` (add custom themes to switcher, set default)

**Step 1: Update ThemeScript default**

In `src/components/ThemeScript.tsx`, change the fallback theme from `'light'` to `'hatcoatandboots-dark'`.
...
Run: `docker compose exec hatcoatandboots npx storybook build --test 2>&1 | tail -10`
Expected: Clean build
...
git commit -m "feat: set hatcoatandboots-dark as default theme"
```
...
**Step 1: Add hatcoatandboots-dark to map control selectors**

The existing CSS has selectors like `[data-theme='dark'] .maplibregl-ctrl-group`. Add `[data-theme='hatcoatandboots-dark']` to each group of dark theme selectors for both MapLibre and Leaflet controls.
...
cd ~/repos/HatCoatAndBoots
git add src/app/globals.css
git commit -m "fix: add hatcoatandboots-dark to map control dark theme selectors"
```
...
Run: `docker compose exec hatcoatandboots npx storybook dev -p 6006 --no-open`

**Step 2: Review Button in both themes**

Open `http://localhost:6006` and switch between hatcoatandboots-dark and hatcoatandboots-light themes. Document what needs to change:
...
git commit -m "style: update Button component for HatCoatAndBoots theme"
```
...
- **Task 22:** AnimatedLogo / SpinningLogo (HatCoatAndBoots's mallet)

### Tasks 23-24: HatCoatAndBoots-specific components
...
- Phase 3 (theme template system) is unique to HatCoatAndBoots as a template project.
