...
Establish a formal process for how the HatCoatAndBoots assembly line takes on external client projects while maintaining internal work. This includes:
...
- Keeps internal HatCoatAndBoots work separate from client deliverables
- Allows parallel work on multiple projects
- Maintains HatCoatAndBoots quality standards for client output
...
| Restriction | Cannot modify HatCoatAndBoots terminals, cannot push         |
...
```
tmux sessions:
├── hatcoatandboots    # 26 terminals (Main Operator)
...
2. Present RFC-009 to Council (in hatcoatandboots session)
...
Keep all work in `hatcoatandboots` session with client prefixes on window names.
...
Fork HatCoatAndBoots for each client project.
...
SpokeToWork is ready for the assembly line. We need a clean way to onboard them without disrupting HatCoatAndBoots internal work.
...
2. **Architect**: Any concerns about the session isolation approach? Should client sessions use a subset of HatCoatAndBoots terminals or completely independent configurations?
...
5. **DevOps**: Should client sessions have their own CI/CD triggers, or share HatCoatAndBoots's?

6. **ProductOwner**: How do we handle client requirements that conflict with HatCoatAndBoots standards?
