...
Establish a formal process for how the HatsCoatsAndBoots assembly line takes on external client projects while maintaining internal work. This includes:
...
- Keeps internal HatsCoatsAndBoots work separate from client deliverables
- Allows parallel work on multiple projects
- Maintains HatsCoatsAndBoots quality standards for client output
...
| Restriction | Cannot modify HatsCoatsAndBoots terminals, cannot push       |
...
```
tmux sessions:
├── hatscoatsandboots    # 26 terminals (Main Operator)
...
2. Present RFC-009 to Council (in hatscoatsandboots session)
...
Keep all work in `hatscoatsandboots` session with client prefixes on window names.
...
Fork HatsCoatsAndBoots for each client project.
...
SpokeToWork is ready for the assembly line. We need a clean way to onboard them without disrupting HatsCoatsAndBoots internal work.
...
2. **Architect**: Any concerns about the session isolation approach? Should client sessions use a subset of HatsCoatsAndBoots terminals or completely independent configurations?
...
5. **DevOps**: Should client sessions have their own CI/CD triggers, or share HatsCoatsAndBoots's?

6. **ProductOwner**: How do we handle client requirements that conflict with HatsCoatsAndBoots standards?
