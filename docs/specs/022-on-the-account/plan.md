...
**Constitution File**: `.specify/memory/constitution.md` (HatsCoatsAndBoots Constitution v1.0.1 - 6 core principles)
...
# Verify
docker compose exec hatscoatsandboots npx supabase db execute \
...
```bash
docker compose exec hatscoatsandboots pnpm add react-easy-crop
```
...
```bash
# Unit tests
docker compose exec hatscoatsandboots pnpm test src/lib/avatar
docker compose exec hatscoatsandboots pnpm test src/components/atomic/AvatarUpload

# E2E tests
docker compose exec hatscoatsandboots pnpm run test:e2e
