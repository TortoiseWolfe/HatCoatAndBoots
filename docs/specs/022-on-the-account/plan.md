...
**Constitution File**: `.specify/memory/constitution.md` (HatCoatAndBoots Constitution v1.0.1 - 6 core principles)
...
# Verify
docker compose exec hatcoatandboots npx supabase db execute \
...
```bash
docker compose exec hatcoatandboots pnpm add react-easy-crop
```
...
```bash
# Unit tests
docker compose exec hatcoatandboots pnpm test src/lib/avatar
docker compose exec hatcoatandboots pnpm test src/components/atomic/AvatarUpload

# E2E tests
docker compose exec hatcoatandboots pnpm run test:e2e
