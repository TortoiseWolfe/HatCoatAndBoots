...
docker compose exec hatscoatsandboots pnpm add chokidar remark remark-html rehype-highlight crypto-js
docker compose exec hatscoatsandboots pnpm add -D @types/crypto-js

# Run migration
docker compose exec hatscoatsandboots pnpm run migrate:blog
```
...
docker compose exec hatscoatsandboots pnpm run test:migration
```
...
docker compose exec hatscoatsandboots pnpm run test:perf
...
```bash
# Test command
docker compose exec hatscoatsandboots pnpm test src/tests/integration/auto-generation.test.ts
```
...
```bash
# Test command
docker compose exec hatscoatsandboots pnpm test src/tests/integration/offline-sync.test.ts
```
...
```bash
# Test command
docker compose exec hatscoatsandboots pnpm test src/tests/integration/conflict-detection.test.ts
```
...
```bash
# Test command
docker compose exec hatscoatsandboots pnpm test src/tests/integration/migration.test.ts
```
...
```bash
# Test command
docker compose exec hatscoatsandboots pnpm test src/tests/integration/enhanced-processing.test.ts
```
...
docker compose exec hatscoatsandboots pnpm run dev:restart

# Check logs
docker compose logs hatscoatsandboots | grep chokidar
```
