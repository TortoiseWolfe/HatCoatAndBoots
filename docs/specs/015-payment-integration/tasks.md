...
# Initialize project
cd /home/turtle_wolfe/repos/HatsCoatsAndBoots
supabase init
...
```bash
docker compose exec hatscoatsandboots pnpm add @supabase/supabase-js @stripe/stripe-js dexie resend

# Dev dependencies for testing
docker compose exec hatscoatsandboots pnpm add -D @supabase/supabase-js @types/stripe stripe
```
...
docker compose exec hatscoatsandboots npx supabase gen types typescript --local > src/lib/supabase/types.ts
```
...
docker compose exec hatscoatsandboots supabase db seed
```
...
docker compose exec hatscoatsandboots supabase db inspect
```
...
docker compose exec hatscoatsandboots supabase secrets set STRIPE_SECRET_KEY=sk_test_xxx
docker compose exec hatscoatsandboots supabase secrets set STRIPE_WEBHOOK_SECRET=whsec_xxx
docker compose exec hatscoatsandboots supabase secrets set PAYPAL_CLIENT_SECRET=xxx
docker compose exec hatscoatsandboots supabase secrets set RESEND_API_KEY=re_xxx

# List secrets (values hidden)
docker compose exec hatscoatsandboots supabase secrets list
```
...
```bash
docker compose exec hatscoatsandboots pnpm run generate:component PaymentButton -- \
  --category atomic \
  --hasProps true \
  --withHooks true
```
...
```bash
docker compose exec hatscoatsandboots pnpm run generate:component PaymentConsentModal -- \
  --category atomic \
  --hasProps true
```
...
```bash
docker compose exec hatscoatsandboots pnpm run generate:component PaymentDashboard -- \
  --category organisms \
  --hasProps false \
  --withHooks true
```
...
```bash
docker compose exec hatscoatsandboots pnpm run generate:component PaymentHistory -- \
  --category molecular \
  --hasProps true
```
...
```bash
docker compose exec hatscoatsandboots pnpm run generate:component SubscriptionCard -- \
  --category molecular \
  --hasProps true
```
...
# Run Lighthouse
docker compose exec hatscoatsandboots npx lighthouse http://localhost:3000/dashboard/payments --output=html --output-path=./lighthouse-report.html
```
