...
**Issue**: [#24](https://github.com/TortoiseWolfe/HatCoatAndBoots/issues/24)
...
2. **`docsUrl` lands on a missing anchor.** Default points at
   `https://github.com/TortoiseWolfe/HatCoatAndBoots#supabase-setup`,
...
- **Localizing the banner copy.** HatCoatAndBoots doesn't have an i18n
...
1. **Unit tests pass:** `docker compose exec hatcoatandboots pnpm test SetupBanner`
2. **A11y tests pass:** `docker compose exec hatcoatandboots pnpm test SetupBanner.accessibility`
...
4. **Storybook:** `docker compose exec hatcoatandboots pnpm run storybook` —
