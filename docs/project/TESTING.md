...
# Run tests with UI
docker compose exec hatcoatandboots pnpm test:ui
...
docker compose exec hatcoatandboots pnpm test tests/integration/auth/ -- --run

# Run contract tests (some will fail - TDD placeholders)
docker compose exec hatcoatandboots pnpm test tests/contract/ -- --run
```
...
```bash
# .husky/pre-commit
docker compose exec -T hatcoatandboots pnpm lint-staged
```
...
| Admin | admin@hatcoatandboots.com | Welcome messages |
...
docker compose exec hatcoatandboots pnpm exec tsx scripts/seed-connections.ts
```
...
docker compose exec hatcoatandboots pnpm exec playwright test tests/e2e/messaging/

# Run with existing dev server (faster)
SKIP_WEBSERVER=true docker compose exec -e SKIP_WEBSERVER=true hatcoatandboots pnpm exec playwright test

# Run specific browser
docker compose exec hatcoatandboots pnpm exec playwright test --project=chromium
...
docker compose exec hatcoatandboots node -e "
...
docker compose exec hatcoatandboots pnpm exec playwright test --trace on

# View trace
docker compose exec hatcoatandboots pnpm exec playwright show-trace test-results/*/trace.zip

# Run in debug mode (step through)
docker compose exec hatcoatandboots pnpm exec playwright test --debug
```
