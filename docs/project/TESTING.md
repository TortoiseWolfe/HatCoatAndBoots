...
# Run tests with UI
docker compose exec hatscoatsandboots pnpm test:ui
...
docker compose exec hatscoatsandboots pnpm test tests/integration/auth/ -- --run

# Run contract tests (some will fail - TDD placeholders)
docker compose exec hatscoatsandboots pnpm test tests/contract/ -- --run
```
...
```bash
# .husky/pre-commit
docker compose exec -T hatscoatsandboots pnpm lint-staged
```
...
| Admin | admin@hatscoatsandboots.com | Welcome messages |
...
docker compose exec hatscoatsandboots pnpm exec tsx scripts/seed-connections.ts
```
...
docker compose exec hatscoatsandboots pnpm exec playwright test tests/e2e/messaging/

# Run with existing dev server (faster)
SKIP_WEBSERVER=true docker compose exec -e SKIP_WEBSERVER=true hatscoatsandboots pnpm exec playwright test

# Run specific browser
docker compose exec hatscoatsandboots pnpm exec playwright test --project=chromium
...
docker compose exec hatscoatsandboots node -e "
...
docker compose exec hatscoatsandboots pnpm exec playwright test --trace on

# View trace
docker compose exec hatscoatsandboots pnpm exec playwright show-trace test-results/*/trace.zip

# Run in debug mode (step through)
docker compose exec hatscoatsandboots pnpm exec playwright test --debug
```
